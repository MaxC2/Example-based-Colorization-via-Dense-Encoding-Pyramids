...
class Split_ab(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        top[0].reshape(self.N,2,self.X,self.Y)

    def forward(self, bottom, top):
        top[0].data[:,0,:,:] = bottom[0].data[:,1,:,:]
        top[0].data[:,1,:,:] = bottom[0].data[:,2,:,:]
...
class Hist_reblance(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.sigma=5
        self.ENC_DIR = './resources/'
        self.reb_hist = np.load(os.path.join(self.ENC_DIR,'prior_probs.npy'))
    ...
    def forward(self, bottom, top):
        ...
        wts = np.exp(-self.reb_hist)
        wts = wts/np.sum(wts)[na()]
        top[0].data[...] = bottom[0].data[...]*wts[np.newaxis,:,np.newaxis,np.newaxis]
...
class NNEncode():
    ...
    def L1_encode_points_mtx_nd(self,pts_nd,axis=1,returnSparse=False,sameBlock=True):
        pts_flt = flatten_nd_array(pts_nd,axis=axis)
        P = pts_flt.shape[0]
        ...
        wts = np.exp(-dists**2/(2*self.sigma**2))
        wts = wts/np.sum(wts,axis=1)[:,na()]
        pts_enc_flt=np.concatenate((inds*0.4/313.0,wts),axis=1)
        # print(pts_enc_flt[[4,5,6,7,8]])
